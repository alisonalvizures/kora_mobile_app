...
import 'package:kora_app/widget/volunteer_item.dart';
import 'colors.dart';
...
class ListVolunterScreen extends StatefulWidget {
  const ListVolunterScreen({super.key});

  @override
  State<ListVolunterScreen> createState() => _ListVolunterScreenState();
}

class _ListVolunterScreenState extends State<ListVolunterScreen>
    ...
    body: Consumer<FirebaseProvider>(
        builder: (context, value, child) {
          return ListView.separated(
            ...
            itemCount: value.volunteers.length,
            separatorBuilder: (context, index) =>
            const SizedBox(height: 10),
            physics: const BouncingScrollPhysics(),
            itemBuilder: (context, index) => value
                .volunteers[index].uid !=
                FirebaseAuth.instance.currentUser?.uid
                ? VolunteerItem(user: value.volunteers[index])
                : const SizedBox(),
          );
        }),
  );
